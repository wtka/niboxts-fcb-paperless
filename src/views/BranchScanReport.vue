...
const totalScanNum = computed(() => {
  const sd = scanData.value;
  let totalNum = 0;
  for (let i = 0; i < sd.length; i++) {
    totalNum += parseInt(sd[i].totalCount);
  }
  return totalNum;
});
  ...
  if (form.checkValidity()) {
    ...
    try {
      ...
      if (process.env.NODE_ENV == 'development') {
        const tmpres = await axios.get('/test/scanreport.json', {
          responseType: 'json',
        });
        res = tmpres.data;
      }
      else {
        const tmpres = await axios.post('/nibox/OneBank/TimerPeriodScan.jsp', params, {
          responseType: 'json',
        });
        res = tmpres.data;
      }
      ...
      pdfURL.value = '/nibox/OneBank/TimerPeriodScan.jsp?' + params_pdf;
      ...
      excelURL.value = '/nibox/OneBank/TimerPeriodScan.jsp?' + params_excel;

      // 掃描結果
      scanData.value = res.PostDatas;

      // 總分頁數
      totalPage.value = Math.ceil(scanData.value.length / pageLimit).toString();
      // 切換當前分頁
...
<template>
  ...
  <!-- 頁面標題 -->
  <h1 class="text-center p-3">分行各時段掃描量統計表</h1>
          ...
          <tr>
            ...
            <td>
              <select class="form-select" v-model='queryProperties'>
                <option v-for="item in dbData" :value="item.name" :key="item.code">{{ item.name }}</option>
              </select>
            </td>
          </tr>
        </tbody>
  ...
  <div v-show="queryResult">
    ...
    <div class="table-responsive d-flex justify-content-center">
      <table class="table table-bordered text-center">
        ...
        <tbody class="text-nowrap">
          <tr class="align-middle" v-for="(item, Index) in scanData" :key="item.Index"
        ...
        <tfoot v-if="currentIndex == parseInt(totalPage)">
          <tr>
            <td colspan="2">總數量</td>
            <td>{{ totalScanNum }}</td>
          </tr>
    ...
    <div class="d-flex justify-content-end">
      <Pagination :queryCount="scanData.length" :currentIndex="currentIndex" :totalPage="totalPage"
