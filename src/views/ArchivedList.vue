...
// 查詢分行各時段掃描量統計表
const archivedData = ref<archived[]>([]);
...
async function query() {
  ...
  if (form.checkValidity()) {
    ...
    try {
      ...
      // 查詢
      const tmpres = await axios.post('/nibox/OneBank/BranchOwnerFileList.jsp', params, {
        responseType: 'json',
      });
      res = tmpres.data;
      ...
      pdfURL.value = '/nibox/OneBank/BranchOwnerFileList.jsp?' + params_pdf;
      ...
      excelURL.value = '/nibox/OneBank/BranchOwnerFileList.jsp?' + params_excel;

      // 掃描結果
      archivedData.value = res.PostDatas;

      // 總分頁數
      totalPage.value = Math.ceil(archivedData.value.length / pageLimit).toString();
      // 切換當前分頁
...
<template>
  ...
  <!-- 頁面標題 -->
  <h1 class="text-center p-3">分行歸檔清單報表</h1>
  ...
  <div v-show="queryResult">
      ...
      <table class="table table-bordered text-center">
        <thead class="text-nowrap">
          <tr class="bg-table">
            ...
            <th scope="col">掃描日期</th>
            <th scope="col">類別 (文件大類)</th>
            <th scope="col">備註</th>
          </tr>
        </thead>
        <tbody class="text-nowrap">
          <tr class="align-middle" v-for="(item, Index) in archivedData" :key="item.Index"
    ...
    <div class="d-flex justify-content-end">
      <Pagination :queryCount="archivedData.length" :currentIndex="currentIndex" :totalPage="totalPage"
